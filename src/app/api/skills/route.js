...
export async function DELETE(req) {
  try {
    ...
    const skillDoc = await Skills.findOne(); // your Skill model
    if (!skillDoc) return NextResponse.json({ message: "Not found" }, { status: 404 });

    if (type === "others") {
      skillDoc.others.splice(index, 1);
    } else {
      skillDoc[type].splice(index, 1);
    }

    await skillDoc.save();

    return NextResponse.json(skillDoc, { status: 200 });
  } catch (err) {
