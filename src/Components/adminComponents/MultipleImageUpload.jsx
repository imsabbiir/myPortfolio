...
function MultipleImageUpload({ onFilesChange, existingImages = [], onExistingImageDelete }) {
    ...
    <div className="p-4 w-full">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {previewImages.map((img, idx) => (
          <div key={idx} className="relative w-24 h-24 group rounded overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-center opacity-0 group-hover:opacity-100 transition">
              <FaTrash className="mx-auto cursor-pointer" onClick={() => handleDelete(idx)} />
            </div>
          </div>
        ))}
        ...
        {existingImages.map((img, idx) => (
          <div key={idx} className="relative w-24 h-24 group rounded overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-center opacity-0 group-hover:opacity-100 transition">
              <FaTrash className="mx-auto cursor-pointer" onClick={() => onExistingImageDelete(img)} />
            </div>
          </div>
        ))}
      </div>
